...
export const Experience = () => {
  ...
  const NewYork = () => {
    const { scene } = useGLTF('./models/city/NewYork-City-Manhattan.glb');
    return <primitive object={scene} />;
  };

  useGLTF.preload('./models/city/NewYork-City-Manhattan.glb')
  ...
  const crossCloudPos = curve.getPointAt(0.8);
  ...
  const cubeVisible = useRef(false);
  ...
  useFrame((_state, delta) => {
    ...
    const targetY = THREE.MathUtils.lerp(0.1, -1, lerpedScrollOffset);
    airplane.current.position.y = THREE.MathUtils.lerp(
    ...
    if (lerpedScrollOffset >= 0.8 && !cubeVisible.current) {
      cubeVisible.current = true;
    }
  
  
    // Show/hide cube based on scroll progress (60% = 0.6)
    if (scrollOffset >= 0.78 && !cubeVisible.current) {
      cubeVisible.current = true;
      cubeRef.current.visible = true;
    } else if (scrollOffset < 0.78 && cubeVisible.current) {
      cubeVisible.current = false;
  ...
  const position = curve.getPointAt(0.85);
  position.y -= 10;
  position.x -= 30;
  ...
  ref={cubeRef} 
  position={position} 
  scale={[50, 40, 50]} // Adjust scale as needed
